...
// （source 値がどう目的地のパラメータ値に作用するか）を定義していない。P1-11
// では 次を暫定で定め、本ファイルで解析的に検証する（理由と根拠は PR
// 本文・Q-018）。
//
// - 加算方式: dest(t) = base(t) + mod(t), ただし mod(t) = source(t) ×
// route.depth
...
// - transient.duration
// はレンダ長そのものを決めるため時間変調の対象外（PresetError）
...
ModulationSourceLfo makeLfoSource(const std::string &id,
                                  const std::string &waveform, double rate,
                                  double depth, bool sync = false) {
...
TEST_CASE(
    "LFO sine source evaluates to waveform over depth (sample-rate free)") {
  const auto src = makeLfoSource("src1", "sine", 1.0, 0.5);
  // phase = 2π·1.0·t。sin が位相の整数倍で真の 0
  // を返さないのは浮動小数点の自然な 挙動（~1e-17）のため、0
  // への一致はマージン付きで判定する。
  REQUIRE(evalModulationSource(src, 0.0) == Approx(0.0).margin(1e-9));
  REQUIRE(evalModulationSource(src, 0.25) == Approx(0.5)); // sin(π/2)*0.5
  REQUIRE(evalModulationSource(src, 0.5) ==
          Approx(0.0).margin(1e-9));                        // sin(π)=0
...
TEST_CASE("LFO triangle/square/saw evaluate to expected normalised values") {
  ...
  REQUIRE(evalModulationSource(tri, 0.125) ==
          Approx(-0.5)); // 1-4*|0.125-0.5|=1-1.5=-0.5
...
TEST_CASE("Envelope source linearly interpolates breakpoints with constant "
          "extrapolation") {
  ...
  REQUIRE(evalModulationSource(env, 2.0) ==
          Approx(1.0)); // constant extrapolation
...
TEST_CASE("Envelope constant source adds source*depth to the destination") {
  ...
  const Preset p =
      parsePreset(presetWithRoute("harmonic.f0", env, 0.5, "linear"));
  ...
  for (double v : it->second)
    REQUIRE(v == Approx(1.0)); // 2.0 × 0.5
}

TEST_CASE("Offsets length matches sample_count") {
  const auto env = makeEnvSource("e", {{0.0, 1.0}});
  const Preset p =
      parsePreset(presetWithRoute("formant.bands[0].freq", env, 1.0, "linear"));
...
TEST_CASE("LFO offset is sample-rate independent (same offset at same t)") {
  ...
  const Preset p = parsePreset(
      presetWithRoute("harmonic.inharmonicity", src, 1.0, "linear"));
  ...
  // t=0.25s, 0.5s に対応する index での変調量が一致（phase=2π·4·t
  // なのでレート非依存）
...
TEST_CASE(
    "route targeting transient.duration is rejected as non-time-modulatable") {
  const auto env = makeEnvSource("e", {{0.0, 1.0}});
  const Preset p =
      parsePreset(presetWithRoute("transient.duration", env, 1.0, "linear"));
  // パス自体は routing で実在と認められる（routing
  // テスト済み）が、意味検証で弾く
...
TEST_CASE(
    "render: constant envelope modulates harmonic f0 from 220 to 264 Hz") {
...
TEST_CASE("render: modulation is deterministic across repeated calls") {
  const auto env = makeEnvSource("e", {{0.0, 1.0}});
  const Preset p = parsePreset(
      presetWithRoute("harmonic.inharmonicity", env, 0.2, "linear"));
  REQUIRE(render(p, 44100.0) == render(p, 44100.0));
}

TEST_CASE("render: route to a disabled layer produces no contribution (stays "
          "silent)") {
  const auto env = makeEnvSource("e", {{0.0, 1.0}});
...
// Q-018 item4「*_db（transient.gain 等）は dB
// 値に加算してから振幅へ変換」の単体検証。 transient.gain への変調が dB
// 空間で加算されること（振幅の乗算 10^(mod/20) になること）を、 renderTransient
// のオフセット経路で確認する。
TEST_CASE("transient.gain modulation is added in dB (amplitude scales by "
          "10^(depth/20))") {
  const auto env = makeEnvSource("e", {{0.0, 1.0}}); // 定数 1.0
  nlohmann::json j =
      presetWithRoute("transient.gain", env, 12.0, "linear"); // depth 12 dB
  j["layers"]["transient"]["enabled"] =
      true; // transient を有効化（既定は有効）
  ...
  for (std::size_t i = 0; i < base.size(); ++i) {
    if (std::fabs(base[i]) < 1e-12)
      continue; // 無音近傍は比率が不定のため飛ばす
