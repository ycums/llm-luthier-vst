...
//
// 検証方針は docs/06-open-questions.md Q-013
// の「既知解フィクスチャに対する解析的な 検証」。エンジンC++単体で、既知の freq
// / Q / gain・既知の遷移を与えて、出力の
// 共振ピーク位置・帯域制御・ゲイン反映・時変遷移・係数急変時の安定性・バイパス・
// サンプルレート非依存・決定論を検証する（Q-010 の stft-peak-tracking
// は指標側の
// 方式でありエンジン側の生成とは独立。この層の検証のために推定方式は選び直さない、
// Issue #55参照）。
//
// 共振フィルタには TPT 状態変数バンドパスを採用する。freq / Q / gain
// はサンプル毎に
...
std::vector<double> sine(double f, int n, double sr) {
  ...
  for (int i = 0; i < n; ++i)
    x[i] = std::sin(2.0 * kPi * f * double(i) / sr);
  return x;
}
...
TEST_CASE("formant: output is bit-identical across repeated calls") {
  const double sr = 44100.0;
  const int n = sr / 2;
  const FormantLayer l =
      layer({band(1000.0, 5.0, 3.0), band(2000.0, 8.0, -2.0),
...
TEST_CASE("formant: time-varying coefficients stay finite and bounded under "
          "abrupt jumps") {
  const double sr = 44100.0;
  ...
  for (std::size_t i = 0; i < y.size(); ++i) {
    ...
    if (i > 0)
      maxJump = std::max(maxJump, std::fabs(y[i] - y[i - 1]));
  }
