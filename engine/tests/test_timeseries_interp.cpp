// docs/03-preset-format.md「時系列の表現」の Timeseries 補間（interp）のテスト。
//
...
Timeseries makeTs(std::initializer_list<std::pair<double, double>> pts, Interp interp) {
    Timeseries ts;
    ts.interp = interp;
    for (const auto& [t, v] : pts) ts.points.push_back(TimeseriesPoint{t, v});
    return ts;
}
...
TEST_CASE("interp holds the endpoint value outside the point range (constant extrapolation)") {
...
TEST_CASE("single-point timeseries is constant for every interp type (docs/03)") {
    for (Interp interp :
...
TEST_CASE("exp interp is geometric (log-domain linear) between two positive points") {
...
TEST_CASE("exp interp falls back to linear when an endpoint is non-positive (exp undefined)") {
...
TEST_CASE("step interp holds the value up to each breakpoint") {
    ...
    REQUIRE(sampleTimeseries(ts, 0.5) == Approx(3.0));   // ブレークポイントで新しい値へ跳ねる
