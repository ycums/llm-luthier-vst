...
// docs/03「meta はエンジンの動作に影響してはならない」（Issue #51
// 完了条件）のテスト。 加えて、統合したレンダパイプライン（P1-08 Harmonic
// 加算合成 / P1-09 Transient/
...
// test_render.cpp を、スタック統合（P1-09 を P1-08
// に積む）の衝突解消で1つに併合した
// もの。両者のテストをすべて保持し、重複する名前（allZero
// ヘルパ、computeRenderDuration の同一テスト）は整理して統合した。
...
// ビット一致は保証し、コンパイラ間のビット一致は保証しない、P1-08
// のPR本文）。その差 は相対 ~1e-15
// 程度であり、絶対値1e-6は周波数・倍音・補間の取り違えという実装誤りと
// 明確に区別できる十分に小さい閾値である。
...
double segmentRms(const std::vector<double> &s, std::size_t begin,
                  std::size_t end) {
  if (end <= begin || end > s.size())
    return 0.0;
  double acc = 0.0;
...
nlohmann::json harmonicPresetJson(const TsSpec &f0,
                                  const std::vector<TsSpec> &partials,
...
Timeseries dbBand(std::vector<std::pair<double, double>> pts) {
  ...
  for (auto &kv : pts)
    t.points.push_back(TimeseriesPoint{kv.first, kv.second});
  return t;
}

TransientLayer singleBandLayer(double durationMs, double gainDb,
                               std::int64_t seed, const Timeseries &env) {
  TransientLayer t;
  t.enabled = true;
...
// 全サンプルを解析的に期待される値と絶対誤差で照合する。`expected` は (t, n) ->
// double。
template <typename F>
void checkAllAgainst(const std::vector<double> &samples, double sampleRate,
                     F expected, double tol = kAbsTol) {
...
TEST_CASE(
    "renderSilence produces only zero samples regardless of enabled flags") {
  nlohmann::json j = minimalValidPreset();
...
TEST_CASE("renderSilence is deterministic across repeated calls (same preset, "
          "same seed)") {
...
TEST_CASE("renderSilence sample count scales with sample rate (sample-rate "
          "independence)") {
  Preset preset = parsePreset(minimalValidPreset());
  ...
  for (double sr : {22050.0, 44100.0, 48000.0, 96000.0}) {
    ...
    const auto expectedCount =
        static_cast<std::size_t>(std::llround(durationS * sr));
...
TEST_CASE("render of multiple constant partials sums their sine components") {
  ...
  Preset preset = parsePreset(harmonicPresetJson(
      {"linear", {{0.0, 220.0}}},
      {{"linear", {{0.0, a1}}}, {"linear", {{0.0, a2}}}}, 0.0, 50.0));
  auto samples = render(preset, fs);
  checkAllAgainst(samples, fs, [&](double t, std::size_t) {
    return a1 * std::sin(kTwoPi * 220.0 * t) +
           a2 * std::sin(kTwoPi * 440.0 * t);
  });
}

TEST_CASE("render f0 tracks a time-varying f0 (glide) with instantaneous freq "
          "f0(t)") {
  const double fs = 48000.0;
  const double dur = 0.05;
  const double fStart = 220.0;
  const double fEnd = 440.0;
  Preset preset = parsePreset(
      harmonicPresetJson({"linear", {{0.0, fStart}, {dur, fEnd}}},
                         {{"linear", {{0.0, 1.0}}}}, 0.0, dur * 1000.0));
  auto samples = render(preset, fs);
  checkAllAgainst(samples, fs, [&](double t, std::size_t) {
    const double phi =
        kTwoPi * (fStart * t + ((fEnd - fStart) / (2.0 * dur)) * t * t);
    return std::sin(phi);
  });
}
...
TEST_CASE("time-varying partial amplitude alters the rendered level") {
  ...
  Preset preset = parsePreset(harmonicPresetJson(
      {"linear", {{0.0, 220.0}}},
      {{"linear", {{0.0, 1.0}}}, {"linear", {{0.0, 0.0}, {dur, 1.0}}}}, 0.0,
      dur * 1000.0));
  auto samples = render(preset, fs);
  checkAllAgainst(samples, fs, [&](double t, std::size_t) {
    const double amp2 = t / dur;
    return 1.0 * std::sin(kTwoPi * 220.0 * t) +
           amp2 * std::sin(kTwoPi * 440.0 * t);
  });
}

TEST_CASE(
    "inharmonicity shifts non-fundamental partials off integer multiples") {
  const double fs = 44100.0;
  const double inh = 0.5;
  Preset preset = parsePreset(harmonicPresetJson(
      {"linear", {{0.0, 220.0}}},
      {{"linear", {{0.0, 0.5}}}, {"linear", {{0.0, 0.5}}}}, inh, 50.0));
  auto samples = render(preset, fs);
  checkAllAgainst(samples, fs, [&](double t, std::size_t) {
...
TEST_CASE(
    "render with harmonic disabled is silent (layer contributes nothing)") {
  nlohmann::json j = minimalValidPreset();
  j["layers"]["transient"]["enabled"] = false;
  j["layers"]["formant"]["enabled"] = false;
  j["layers"]["harmonic"]["enabled"] = false;
...
TEST_CASE(
    "render is deterministic across repeated calls (same preset, same seed)") {
  Preset preset = parsePreset(minimalValidPreset());
...
TEST_CASE(
    "render sample count scales with sample rate (sample-rate independence)") {
  Preset preset = parsePreset(minimalValidPreset());
  const double durationS = computeRenderDurationSeconds(preset);
  REQUIRE(durationS > 0.0);
  for (double sr : {22050.0, 44100.0, 48000.0, 96000.0}) {
...
TEST_CASE("renderTransient: enabled single-band produces non-zero samples of "
          "the duration's length") {
  const auto s = renderTransient(
      singleBandLayer(3000.0, 0.0, 7, dbBand({{0.0, 0.0}, {3.0, 0.0}})),
      44100.0);
...
TEST_CASE("Transient: gain (dB) scales the output by the exact linear "
          "amplitude ratio") {
  ...
  for (std::size_t i = 0; i < hi.size(); ++i) {
    ...
    REQUIRE(std::fabs(lo[i] - expected) <=
            1e-6 * std::max(1.0, std::fabs(expected)));
  }
}

TEST_CASE("Transient: same seed is reproducible; different seed yields "
          "different noise") {
...
TEST_CASE(
    "render: enabled transient contributes, disabled transient stays silent") {
  nlohmann::json j = minimalValidPreset();
  j["layers"]["harmonic"]["enabled"] = false;
  j["layers"]["formant"]["enabled"] = false;
...
// ---------------------------------------------------------------------------
// 統合：render() が Transient + Harmonic の加算結果に Formant filter bank
// を適用する （docs/02-engine-spec.md 層[3]、P1-10 #55）。※Harmonic
// 単体の解析検証（上記）は formant を無効化してあり、ここでは formant
// の「結線」だけを検証する。
// ---------------------------------------------------------------------------

TEST_CASE(
    "render applies formant filter at the band centre frequency to the mix") {
  const double fs = 44100.0;
  const double kQ = 10.0, kGainDb = 0.0;

  // Harmonic 単一倍音（220Hz 正弦）。transient は無効にする。
  // docs/02 は formant バンド数を v0
  // では4個固定としており、ローダが厳密に実施する
  // （preset_loader）。ここでは4バンドすべてを同一の probe
  // 周波数に置く（並列バンクの 選択性検証として十分）。
  ...
  // formant
  // が有効なときは加算結果に必ず適用される（バイパスと同一にならない）。
  REQUIRE_FALSE(at220 == bypass);
}

TEST_CASE("render with formant is deterministic; toggling formant changes the "
          "output") {
  const double fs = 44100.0;
  nlohmann::json j = minimalValidPreset();
  j["layers"]["transient"]["enabled"] = false;
