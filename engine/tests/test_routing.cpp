...
TEST_CASE("an out-of-range array index is rejected") {
  ...
  REQUIRE_THROWS_AS(presetWithRoute("harmonic.partial_amplitudes[5]"),
                    PresetError);
  REQUIRE_THROWS_AS(presetWithRoute("transient.spectral_envelope[9]"),
                    PresetError);
}
...
TEST_CASE("an unknown parameter name is rejected") {
  ...
  REQUIRE_THROWS_AS(presetWithRoute("formant.bands[0].nonexistent"),
                    PresetError);
}
