...
// 加算方式（docs/02 には適用式の規定がなく、P1-11 で暫定を決めた。理由は PR
// 本文参照）。
//
// - LFO: 波形は 2π·rate·t の位相で [-1,1] に正規化した波形値に、LFO 自身の
//   `depth` を乗算する。`rate` は Hz。`sync` はヘッドレス v0
//   ではノートオンが常に 0.0 から開始するため位相に影響しない（暫定。Q-018）。
//   波形は v0.1 では sine / triangle / square / saw をサポートし、それ以外は
//   PresetError（未知波形を黙って無視しない。docs/adr/0006・P1-09 の規約）。
// - Envelope: ブレークポイント列を linear
// 補間し、範囲外は端点値で定数外挿する。
//   暫定で linear のみ（Q-018）。
...
// - route.curve は v0.1 では linear
// のみ対応。それ以外（exp/step/spline）は未知の
