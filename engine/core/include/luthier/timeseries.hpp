...
inline std::vector<double> splineSecondDerivatives(const std::vector<TimeseriesPoint>& pts) {
    ...
    for (std::size_t i = 1; i + 1 < n; ++i) {
        ...
        rhs[i] = 6.0 * ((pts[i + 1].v - pts[i].v) / h_next - (pts[i].v - pts[i - 1].v) / h_prev);
    }
    ...
    for (long k = static_cast<long>(n) - 3; k >= 1; --k) {
        M[static_cast<std::size_t>(k)] =
            (rhs[static_cast<std::size_t>(k)] - super[static_cast<std::size_t>(k)] * M[static_cast<std::size_t>(k) + 1]) /
...
inline double sampleTimeseries(const Timeseries& ts, double t) {
    ...
    const std::size_t n = pts.size();
    if (n == 0) return 0.0;  // 防御的。ローダーが points>=1 を保証（preset_loader）
    ...
    switch (ts.interp) {
        ...
        case Interp::Exp:
            ...
            if (v0 > 0.0 && v1 > 0.0 && v0 != v1) return v0 * std::pow(v1 / v0, u);
