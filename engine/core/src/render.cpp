...
// ---------------------------------------------------------------------------
// 共通の Timeseries 補間（docs/03「時系列の表現」）。3実装（P1-08 の
// sampleTimeseries / P1-09 の evalTimeseries / P1-10 の
// evaluateTimeseries）を、P1-08 で追加された 唯一の補間関数
// `sampleTimeseries`（timeseries.hpp、linear/exp/step/spline 対応）に
// 統一する。P1-09 の「exp/spline は v0.1 では未実装 → 黙って線形化せず
// PresetError で停止する」規約（docs/adr/0009）は、層ごとの policy
// として呼び出し側で維持する（ transientEnvelopeAt）。
// ---------------------------------------------------------------------------

// Transient 層のスペクトル包絡を評価する。exp / spline は v0.1
// では未実装のため、 黙って直線近似に置き換えず PresetError
// で停止する（AGENTS.md 第7節、P1-09 規約）。
double transientEnvelopeAt(const Timeseries &env, double t) {
  if (env.interp != Interp::Linear && env.interp != Interp::Step) {
    throw PresetError(
        "transient spectral_envelope: interp '" +
...
// splitmix64
// を自前実装する。標準ライブラリの分布クラスは実装間で出力が一致しないため、
// コンパイラをまたいだ再現のためには自前実装が必要（Issue
// #54）。ここでは「PRNG自体 （整数64bitの列）は gcc/MSVC
// でビット一致する」ことを満たすため整数演算に限定する。 分布は振幅 [-1,1)
// の一様（白色雑音）。帯域フィルタ後の浮動小数は FMA 融合等で最下位
// ビットが揺れうるため、この層は「同一バイナリ／同一環境内でビット一致」の保証範囲。
// （ADR-0009）。
struct SplitMix64 {
  std::uint64_t state;
  explicit SplitMix64(std::int64_t seed)
...
// dB → 振幅（linear）。
double ampDb(double db) { return std::pow(10.0, db / 20.0); }

// Harmonic周期（倍音k）の周波数[Hz]。inharmonicity の補正式は P1-08 の実装詳細
// （docs/adr/0007「補正式自体はP1-08の実装詳細」、docs/06 Q-016）：
//   freq_k = f0 * k * (1 + inharmonicity * (k - 1))
inline double partialFrequency(const Timeseries &f0, std::size_t k,
                               double inharmonicity, double t) {
  const double base = sampleTimeseries(f0, t);
  return base * static_cast<double>(k) *
         (1.0 + inharmonicity * (static_cast<double>(k) - 1.0));
}

// Harmonic / Body 層の加算合成（docs/adr/0007、P1-08 #53）。サンプル列の長さは
// `sampleCount` で与え、`enabled == false`
// のときは全ゼロ（無音）。位相は「時刻tでの 瞬間角速度が 2π·f_k(t)
// に一致する」よう時間積分で累積する（滞搗則、独立した乱数・
// 履歴状態を持たず同一バイナリ内でビット決定論的）。
std::vector<double> renderHarmonic(const HarmonicLayer &h,
...
// （Q-004 の暫定値。docs/adr/0009）。帯域 b は [b/B, (b+1)/B]×Nyquist
// に対応し、 一極ローパス2段の差分（band = LP@hi -
// LP@lo）で帯域制限した白色ノイズに、その帯域の
// 時間変化する包絡（dB）を振幅スケールして加算する。`duration_ms`
// はこの層の長さ。
std::vector<double> renderTransient(const TransientLayer &layer,
                                    double sample_rate_hz) {
  if (!layer.enabled || layer.spectral_envelope.empty())
    return {};
  if (sample_rate_hz <= 0.0) {
    throw PresetError(
        "transient render: sample_rate_hz は正の数である必要がある");
  }
  ...
  for (std::size_t b = 0; b < B; ++b) {
    aLo[b] =
        lowpassAlpha(static_cast<double>(b) / static_cast<double>(B) * nyquist,
                     sample_rate_hz);
    aHi[b] = lowpassAlpha(static_cast<double>(b + 1) / static_cast<double>(B) *
                              nyquist,
                          sample_rate_hz);
  }

  std::vector<double> yHi(B, 0.0), yLo(B, 0.0);
  const double gain = ampDb(layer.gain_db);
  ...
  for (std::size_t i = 0; i < n; ++i) {
    ...
    for (std::size_t b = 0; b < B; ++b) {
      ...
      const double amp =
          ampDb(transientEnvelopeAt(layer.spectral_envelope[b], t));
      acc += band * amp;
    }
    out[i] = acc * gain;
  }
...
std::vector<double> render(const Preset &preset, double sample_rate_hz) {
  ...
  //      applyFormant は formant.enabled==false
  //      のとき入力をそのまま返す（バイパス）。
  std::vector<double> out =
      renderHarmonic(preset.harmonic, sampleCount, sample_rate_hz);
  const std::vector<double> tr =
      renderTransient(preset.transient, sample_rate_hz);
