...
// 変調オフセット列（P1-11）からの参照。オフセットが無い（nullptr
// またはパス不在） なら 0.0 を返し、加算変化量がゼロとして振る舞う。
...
std::vector<double> renderHarmonic(const HarmonicLayer &h,
  ...
  for (std::size_t n = 0; n < sampleCount; ++n) {
    ...
    for (std::size_t k = 1; k <= numPartials; ++k) {
      ...
      const double f = f0 * static_cast<double>(k) *
                       (1.0 + inh * (static_cast<double>(k) - 1.0));
      const double fNext = f0Next * static_cast<double>(k) *
                           (1.0 + inh * (static_cast<double>(k) - 1.0));
...
std::vector<double> renderTransient(const TransientLayer &layer,
  ...
  for (std::size_t i = 0; i < n; ++i) {
    ...
    const double gainDb =
        layer.gain_db + modOffsetAt(mod_offsets, "transient.gain", i);
...
std::vector<double> render(const Preset &preset, double sample_rate_hz) {
  ...
  // P1-11（#56）：変調オフセットを事前計算する。プリセットに modulation
  // が無い、 またはルートが無い場合は空 map
  // になり、各層でオフセット0として振る舞う。
