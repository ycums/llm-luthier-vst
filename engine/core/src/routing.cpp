...
std::vector<PathSegment> tokenize(const std::string &path) {
  ...
  if (n == 0)
    malformed(path, "空のパスは指定できない");

  while (i < n) {
    if (!isIdentStart(path[i]))
      malformed(path, "識別子の先頭が不正な位置 " + std::to_string(i));
    std::size_t start = i;
    ...
    if (i < n && path[i] == '[') {
      ...
      seg.has_index = true;
      seg.index = static_cast<std::size_t>(
          std::stoull(path.substr(digitsStart, i - digitsStart)));
      ++i; // consume ']'
    }
    ...
    if (i < n) {
      if (path[i] != '.')
        malformed(path, "区切りは '.' のみ許容される");
      ++i;
      if (i >= n)
        malformed(path, "末尾が '.' で終わっている");
    }
  }
  return segments;
}
...
void expectNoIndex(const PathSegment &seg, const std::string &path) {
  if (seg.has_index)
    badPath(path, "'" + seg.name + "' に添字は付けられない");
}

void expectFinal(const std::vector<PathSegment> &segs, std::size_t idx,
                 const std::string &path) {
  if (idx != segs.size() - 1)
    badPath(path, "'" + segs[idx].name + "' より先のパスは存在しない");
}

void resolveTransient(const TransientLayer &layer,
                      const std::vector<PathSegment> &segs, std::size_t idx,
                      const std::string &path) {
  ...
  if (seg.name == "spectral_envelope") {
    if (!seg.has_index)
      badPath(path, "'spectral_envelope' には添字が必要");
    if (seg.index >= layer.spectral_envelope.size()) {
      badPath(path, "'spectral_envelope[" + std::to_string(seg.index) +
                        "]' は範囲外（実際の個数: " +
...
void resolveHarmonic(const HarmonicLayer &layer,
                     const std::vector<PathSegment> &segs, std::size_t idx,
                     const std::string &path) {
  const auto &seg = segs[idx];
  ...
  if (seg.name == "partial_amplitudes") {
    if (!seg.has_index)
      badPath(path, "'partial_amplitudes' には添字が必要");
    if (seg.index >= layer.partial_amplitudes.size()) {
      badPath(path, "'partial_amplitudes[" + std::to_string(seg.index) +
                        "]' は範囲外（実際の個数: " +
...
void resolveFormant(const FormantLayer &layer,
                    const std::vector<PathSegment> &segs, std::size_t idx,
                    const std::string &path) {
  const auto &seg = segs[idx];
  if (seg.name != "bands") {
  ...
  if (!seg.has_index)
    badPath(path, "'bands' には添字が必要");
  if (seg.index >= layer.bands.size()) {
    badPath(path, "'bands[" + std::to_string(seg.index) +
                      "]' は範囲外（実際の個数: " +
  ...
  if (sub.name != "freq" && sub.name != "q" && sub.name != "gain") {
    badPath(path, "'bands[" + std::to_string(seg.index) + "]." + sub.name +
                      "' は存在しない");
  }
...
void validateRoutingPath(const Preset &preset, const std::string &path) {
  ...
  if (segs.size() < 2)
    badPath(path,
            "層名だけでは変調先として不完全（例：formant.bands[0].freq）");
  ...
  if (root.name == "transient") {
    ...
    badPath(path, "未知のルート層 '" + root.name +
                      "'（transient/harmonic/formantのいずれでもない）");
  }
}
...
void validateRoutingPaths(const Preset &preset) {
  ...
  for (const auto &route : preset.modulation->routes) {
    if (!sourceIds.contains(route.from)) {
      throw PresetError("存在しない変調源id '" + route.from +
                        "' を参照するルートがある");
    }
