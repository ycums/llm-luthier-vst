...
void writeU16LE(std::ofstream& out, std::uint16_t v) {
    std::array<char, 2> b{static_cast<char>(v & 0xFF), static_cast<char>((v >> 8) & 0xFF)};
    out.write(b.data(), static_cast<std::streamsize>(b.size()));
}
...
void writeWavFloatMono(const std::filesystem::path& path, const std::vector<double>& samples,
                       std::uint32_t sample_rate_hz) {
    std::ofstream out(path, std::ios::binary | std::ios::trunc);
    if (!out) throw WavWriteError("出力WAVを開けない: " + path.string());
    ...
    const auto byteRate = static_cast<std::uint32_t>(sample_rate_hz) * kChannels * kBytesPerSample;
    ...
    // RIFFサイズ = "WAVE"(4) + fmt チャンク(8+16) + fact チャンク(8+4) + data チャンク(8+dataBytes)
    ...
    if (!out) throw WavWriteError("出力WAVの書き込みに失敗した: " + path.string());
}
