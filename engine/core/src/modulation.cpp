...
double triangleWave(double u) { return 1.0 - 4.0 * std::fabs(u - 0.5); }

double waveformValue(const std::string &waveform, double u) {
  if (waveform == "sine")
    return std::sin(kTwoPi * u);
  if (waveform == "triangle")
    return triangleWave(u);
  if (waveform == "square")
    return u < 0.5 ? 1.0 : -1.0;
  if (waveform == "saw")
    return 2.0 * u - 1.0;
  throw PresetError("未知の LFO 波形 '" + waveform +
                    "'（v0.1 対応: sine/triangle/square/saw、Q-018）");
}

// ルートのカーブ。v0.1 では linear のみ。それ以外は推測で埋めず PresetError
// で停止 （docs/adr/0006・P1-09 の規約、Q-018）。
void requireLinearCurve(Interp curve, const std::string &to) {
  if (curve != Interp::Linear) {
    throw PresetError(
        "route の curve は v0.1 では linear のみ対応（destination '" + to +
  ...
  return std::visit(
      [t](const auto &s) -> double {
        ...
        if constexpr (std::is_same_v<T, ModulationSourceLfo>) {
          ...
          // v0 ではノートオンが常に 0.0 から始まり、tempo/clock
          // が存在しないため 位相に影響しない（Q-018）。
          const double phaseU =
...
buildModulationOffsets(const Preset &preset, double sample_rate_hz,
  ...
  for (const auto &src : preset.modulation->sources) {
    sourcesById.emplace(std::visit([](const auto &s) { return s.id; }, src),
                        &src);
  }
  ...
  for (const auto &route : preset.modulation->routes) {
    ...
    if (it == sourcesById.end()) {
      throw PresetError("存在しない変調源id '" + route.from +
                        "' へのルート（routing で検出済みのはず）");
    }
    auto &a = acc[route.to];
    if (a.sources.empty())
      destOrder.push_back(route.to);
