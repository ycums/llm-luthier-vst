"""全体指標（P0-05 #5）と区間別指標（P0-06 #6）の実装。

`docs/04-metrics.md` の「全体指標」「区間別指標」を算出し、`docs/04-metrics.schema.json` に
valid な指標ベクトル全体（overall / segments / bands / trajectories / calc_conditions）を
組み立てる。区間別指標は全体指標と同じ3指標（msstft / mfcc / loudness_diff_db）を、区間境界で
切り出した部分波形に対して算出したものである。

帯域別（P0-07 #7）・軌跡（P0-08 #8）の各指標は本Issueのスコープ外であり、それぞれ欠測
（`value: null` + `missing_reason`）として出力する。estimation_algorithms も同じ理由で
...
_NOT_IMPLEMENTED_REASONS = {
    ...
    "trajectories": "軌跡指標は本Issue（P0-06 #6）のスコープ外（P0-08 #8 で実装予定）",
}
...
def compute_metrics_vector(
    ...
    全体指標（overall）と区間別指標（segments）は本Issue（#5, #6）で実際に算出する。
    区間境界のうち `transition_end_s` / `sustain_end_s` は既定値を持たない外部入力であり、
    与えられなければ該当区間は欠測になる（`_segment_slice_bounds` 参照）。bands /
    trajectories は P0-07 / P0-08 のスコープであり、ここでは欠測として出力する。
    """
