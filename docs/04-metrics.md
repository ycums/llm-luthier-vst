...
- **一致の判定方法：ビット完全一致（許容誤差なし）を採用する（Issue #88で決定）。**
  ...
  - **〔改訂・Issue #107〕ビット完全一致は廃止が決まった（実装は未反映）。** 同一イメージ・同一入力の
    `ubuntu-latest` の2 run で判定が割れた（#96）ため、`docs/adr/0008` 「8.」で、指標値のマシン間の
    ビット一致を保証しないこと、判定を `math.isclose(rel_tol=1e-12, abs_tol=1e-12)` に変えることを
    決定した。本項の記述は実装Issueで書き換える。下記「一致の判定方法の実測（Issue #88）」の
    「環境差による誤検出は起きない」という結論は、ローカル1環境での実測に基づくものであり、#96 で
    覆された
