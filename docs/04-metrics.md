...
## target と candidate の時間整列（ADR-0012）

上記の指標はいずれも、target と candidate を時刻を揃えて比較する。揃え方は次のとおり（`docs/adr/0012-target-candidate-time-alignment.md`、Q-019）。

- **整列はハーネスが持つ。** プリセットの `t` はノートオンを0とする規約（`docs/03-preset-format.md`）のまま変えず、target の先頭無音をプリセットで表現しない
...
（#86「完了条件」）。本決定は、フェーズ1の消化計画 #45 が open のうち（2026-09-06、PR #87）に行われて
おり、この条件を満たしている。フェーズ1の消化計画 #45 はクローズ済みで、フェーズ2の消化計画と判定の
置き場所は #128 である（`docs/00-vision.md`「現在地」）。
